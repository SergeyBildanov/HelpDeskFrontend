...
document.addEventListener("DOMContentLoaded", () => {
  ...
  ticketList.addEventListener("click", e => {
    ...
    if (e.target.closest(".ticket")) {
      if (e.target.closest(".ticket").querySelector(".description").textContent) {
        e.target.closest(".ticket").querySelector(".description").textContent = "";
        return;
      }
      let xhr = new XMLHttpRequest();
      let id = e.target.closest(".ticket").dataset.id;
      xhr.open("GET", `http://localhost:1001?method=ticketById&id=${id}`);
      xhr.addEventListener("load", () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            let description = e.target.closest(".ticket").querySelector(".description");
  ...
  addModal.addEventListener("click", e => {
    ...
    if (e.target.closest(".ok-btn")) {
      ...
      addModal.classList.add("hidden");
      let xhr = new XMLHttpRequest();
      xhr.open("POST", "http://localhost:1001?method=createTicket");
      xhr.addEventListener("load", () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            drawTickets(ticketList);
  ...
  changeModal.addEventListener("click", e => {
    ...
    if (e.target.closest(".ok-btn")) {
      ...
      changeModal.classList.add("hidden");
      let xhr = new XMLHttpRequest();
      xhr.open("POST", `http://localhost:1001?method=changeTicket&id=${changeModal.dataset.id}`);
      xhr.addEventListener("load", () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            drawTickets(ticketList);
  ...
  deleteModal.addEventListener("click", e => {
    ...
    if (e.target.closest(".ok-btn")) {
      ...
      let xhr = new XMLHttpRequest();
      xhr.open("GET", `http://localhost:1001?method=deleteById&id=${changeModal.dataset.id}`);
      xhr.addEventListener("load", () => {
        if (xhr.status >= 200 && xhr.status < 300) {
          try {
            drawTickets(ticketList);
...
function drawTickets(ticketList) {
  ...
  let xhr = new XMLHttpRequest();
  xhr.open("GET", "http://localhost:1001?method=allTickets");
  xhr.addEventListener("load", () => {
    if (xhr.status >= 200 && xhr.status < 300) {
      try {
...
function drawTicket(ticketList, ticket) {
  ticketList.insertAdjacentHTML("beforeEnd", `<div class="ticket" data-id=${ticket.id}>
        <div class="header">
